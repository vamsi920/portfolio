import { useFaceDepth } from '@/context/FaceDepthContext'
import { useSoundFeedback } from '@/context/SoundFeedbackContext'

export function FaceDepthConsent() {
  const { state, acceptConsent, declineConsent } = useFaceDepth()
  ...
  return (
    ...
    >
      <div className="glass-strong mx-4 max-w-md rounded-2xl p-6 shadow-xl">
        <h2 id="depth-consent-title" className="font-display text-lg font-semibold text-white">
          Face-distance depth mode
        </h2>
        <p className="mt-3 font-body text-sm text-white/80">
          Use your camera to estimate distance. The scene will smoothly scale with how close you are
          — lean in to zoom in, lean back to zoom out. All processing stays in your browser.
        </p>
